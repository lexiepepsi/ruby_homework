...
# Instance variables are tied to a particular instance of a class, that is, they are tied to particular objects. They are accessible and visible outside the method where they are defined. We can see and access instance variables when we call specific objects (even if we are outside defining the Class).

# Instance variables are different from local variables which are accessible only in the space they are defined. For example when we define the variable 'a' within the sum method, we can't access a to call sum outside the method.
def sum(entry)
...
# sum(a) 
# This line of code will result in an error because a has not been defined in a place where sum can access it. Ruby thinks a has no value (not even nil) because local variables don't have the value of nil before they are created like instance variables do.
...
class Person
	# Here an instance variable called name will be defined when we initialize a new Person object. Instance variables are defined by using @ before the name of the variable.
...
#Even though we're outside the Person class, the alexa object had the instance variable @name set to "Alexa" when it was initialized. Now, the name method calls the instance variable @name for the alexa object, which is "Alexa". Even though we are not inside the Person class, and have not specified what @name should be in this exact line of code, we can access the instance variable associated with the particular alexa object. Instance variables are tied to their unique objects and can be called on when we are working with the alexa object.
