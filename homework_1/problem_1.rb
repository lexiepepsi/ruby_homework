...
def full_name(first_name, last_name, title)
  ...
  # If the variable tile AND the variable first_name AND the variable last_name are set to a value, run the code below this line.
  ...
  # Else, if ((if the 3 above conditionals did not evaluate to true)), if first_name is set to a variable run the code below.
    ...
    name = first_name
  # Else ((if none of the above conditionals evaluated to true)), that is is if false, run the code below.
  else
  	# Create an error message that shows That doesn't look like a name! in a runtime error alert.
...
# Indicates to ruby that we are done defining the method. We defined a method called full_name that took 3 arguments. We set a variable called name to nil. We ran througha conditional statement that changed the value of name if certain arguments were present. We retunred the value of name at the end of the method and now we are at the end of the method.
end
